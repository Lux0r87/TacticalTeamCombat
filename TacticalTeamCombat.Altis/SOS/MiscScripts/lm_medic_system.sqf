...
lm_MS_const_bleedReduce						= 0.0014;	// Lux0r: 0.00014;
lm_MS_const_bleedReduceMedic				= 0.002;	// Lux0r: 0.0002;
lm_MS_const_bleedTickMax					= 0.02;
lm_MS_const_bleedRegenAmount				= 0.014;	// Lux0r: 0.007;
...
lm_MS_const_incapacitationReportRange		= 100.0;	// Lux0r: changed lm_MS_fnc_reportRegainedConsciousness and lm_MS_fnc_reportIncapacitatedPlayer
lm_MS_const_cardiacArrestDelay				= 60.0;
lm_MS_const_cardiacArrestChance				= 0;		// Lux0r: 0.002;
...
lm_MS_const_wakeChance						= 0.08;		// Lux0r: 0.04;	// 0.05
lm_MS_const_wakeTick						= 10.0;
lm_MS_const_fatalBleedChance				= 0;		// Lux0r: 0.01;	// 0.02;
lm_MS_const_fatalBleedMinDamage				= 0.2;
lm_MS_const_dynamicBlurMultiplier			= 4.0;
lm_MS_const_isTFR							= false;	// Lux0r: true;
...
lm_MS_fnc_reportIncapacitatedPlayer = {
	...
	//if ((player distance _unit) > lm_MS_const_incapacitationReportRange) exitWith {};		// Lux0r: no range limit for public server
	if (alive _unit) then {
...
lm_MS_fnc_doExamineAction = {
	...
	if (!_isAnimCancelled) then {
		...
		player setVariable [lm_MS_var_isTreating, false, true];
		hint parseText format [
			"%1 is %2 wounded<br/>and %3 bleeding.",
			(name _injured),
	...
	if (!_isAnimCancelled) then {
		...
		lm_MS_animHandle = [_injured] spawn {
			...
			while {true} do {			
				...
				if !(_injured getVariable lm_MS_var_isFatallyBleeding) then {
					_reduceAmount = 0.08 + (random 1.0) * 0.08;	// Lux0r: _reduceAmount = 0.02 + (random 1.0) * 0.08;
...
lm_MS_fnc_addMedicActions = {
	...
	if (_unit == player) then {	
		...
		// Lux0r: remove action for public PvP
		...
		while {(alive player) && (player getVariable lm_MS_var_isIncapacitated)} do {			
			...
			if (_internalDamage < (lm_MS_const_internalDamageLimit * 0.5)) then {
				...
				if ((player getVariable lm_MS_var_isIncapacitated) && (player getVariable lm_MS_var_isBreathing) && (_rand <= _wakeUpChance)) then {
					...
					[[player], "lm_MS_fnc_reportRegainedConsciousness", side player, false] call BIS_fnc_MP;	// Lux0r: broadcast the report only to the side of the player
				};	
			};
	...
	[_animChangedEHIndex] spawn {
		...
		if ((alive player) && !(player getVariable lm_MS_var_isIncapacitated)) then {
			...
			if !(isNil "_revivedBy") then {
				[[player, _revivedBy], "lm_MS_fnc_reportRevivedPlayer", side player, false] call BIS_fnc_MP;	// Lux0r: broadcast the report only to the side of the player
